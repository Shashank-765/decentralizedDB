        ...
        Dashboard
            ...
            <li>
              {user ? (
                ...
                <button onClick={handlesignuppopup} className="text-gray-700 hover:text-gray-800 text-lg w-32 h-12 flex items-center justify-center rounded-full bg-white shadow-md hover:shadow-lg transition">Sign Up</button>
              )}
            </li>
          </ul>
    ...
    <li><Link to="/dashboard" className=" mobile-nav-link text-gray-700 hover:text-gray-800 text-lg w-32 h-12 flex items-center justify-center rounded-full bg-white shadow-md hover:shadow-lg transition">Dashboard</Link></li>
            ...
            <li>
              {user ? (
                ...
                <button onClick={() => setIsPopupOpen(true)} className=" mobile-nav-link text-gray-700 hover:text-gray-800 text-lg w-32 h-12 flex items-center justify-center rounded-full bg-white shadow-md hover:shadow-lg transition">Sign Uip</button>
              )}
            </li>
      ...
      {isPopupOpen && (
        ...
        onClick={handleBackgroundClick}>
          ...
          onClick={(e) => e.stopPropagation()}>
            ...
            {activeTab === "signUp" ? <SignUpForm  /> : activeTab ==="forgotpassword" ? <ForgotpasswordForm/> :<SignInForm  />}
             {/* Social Logins */}
 ...
 const cancelopthandler = ()=>{
  ...
  setOtp("");
   ...
   const handleForgotPassword = async(e: React.FormEvent) => {
    ...
    console.log("Sending OTP to email:", emailtoforgot);
    const res = await axios.post(`${config.URL_BACKEND}api/auth/otpverifiucation`, {
      email:emailtoforgot,
    });
    if(res.data) {
      setShowOTPModal(true);
      ToastMessage("OTP sent successfully", "successs", "");
    }
    else{  
      setShowOTPModal(false);
  ...
  const handleVerifyOTP = async () => {
    // TODO: logic to verify OTP
     if (otp.length !== 6) {
       console.error("Invalid OTP length. OTP should be 6 digits.");
 ...
 const newErrors: { [key: string]: string } = {};
    ...
    try {
      ...
      if (res.data) {
        ...
        setConfirmPassword("");
        setActiveTab("signIn");
        ToastMessage("Password changed successfully!", "successs", "");
      }
    } catch (err: any) {
  ...
  >
    <input
      ...
      required
    />
    <button type="submit" className="w-full bg-indigo-500 text-white py-2 rounded-lg" onClick={handleForgotPassword}>
      Send OTP
    </button>
      ...
      <div className="otp-modal-overlay">
        <div className="otp-modal">
          ...
          <div className="otp-actions">
            ...
            <button className="cancel-otp" onClick={cancelopthandler}>
              Cancel
            </button>
          </div>
        </div>
...
{isVerified && (
  ...
  {/* Move new password form block here OUTSIDE of form */}
